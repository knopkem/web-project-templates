...
class ProcessManager(object):
    ...
    def startProcess(self, session):
        ...
        with open(logFilePath, "a+") as log_file:
            try:
                logging.warning(' '.join(map(str, session['cmd']))) 
    ...
    def isReady(self, session, count = 0):
      ...
      logging.warning("ready state: " + str(ready))
      return ready
...
class LauncherResource(resource.Resource, object):
    ...
    def _waitForReady(self, session, request, count=0, d=None):
        logging.warning("waiting for ready")
    ...
    def _delayedRenderTimeout(self, request, session):
        logging.warning("delayedRenderTimeout")
        ready = self.process_manager.isReady(session, 0)

        request.responseHeaders.addRawHeader(b"content-type", b"application/json")

        if ready:
            request.write(json.dumps(filterResponse(session, self.field_filter)).encode())
            request.setResponseCode(http.OK)
        else:
            request.write(json.dumps({"error": "Session did not start before timeout expired. Check session logs."}).encode())
    ...
    def _delayedRenderReady(self, request, session):
        logging.warning("delayedRenderReady")
        filterkeys = self.field_filter
        if session['secret'] in session['cmd']:
            filterkeys = self.field_filter + [ 'secret' ]
        request.responseHeaders.addRawHeader(b"content-type", b"application/json")
        request.write(json.dumps(filterResponse(session, filterkeys)).encode())
        request.setResponseCode(http.OK)

        logging.warning("request finished")
        request.finish()
...
def startWebServer(options, config):
    ...
    logging.warning("Starting launcher webserver")
    ...
    # Attach launcher
    web_resource.putChild(endpoint.encode(), LauncherResource(options, config))
